// topic.js : 컨텐츠에 CRUD에 관한 모듈들을 작성
...
exports.page = function(request, response){
    ...
    // { id : 'HTML' }
     ...
     db.query(`SELECT * FROM topic`, function(error,topics){ // 전체 목록 출력
        ...
        /* SQL INJECTION 공부용 코드 (INJECTION 공격 당하는 코드)
        var query = db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id 
        WHERE topic.id=${queryData.id}`,
        */
        
        /* SQL INJECTION 막는 코드1
        
        var query = db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id 
        WHERE topic.id=${db.escape(queryData.id)}`,
        */
        
        // SQL INJECTION 막는 코드2
